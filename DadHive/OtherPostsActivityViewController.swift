//
//  OtherPostsActivityViewController.swift
//  DadHive
//
...
class OtherPostsActivityViewController: UIViewController {
  ...
  private func setupTopicsTable() {
    ...
    topicsTable.emptyDataSetView { view in
      view.titleLabelString(NSAttributedString(string: "No Activity"))
        .detailLabelString(NSAttributedString(string: "Begin upvoting and commenting on topics to see your activity here"))
  ...
  private func retrieveAllData(completion: @escaping (NotificationResponse?, Error?) -> Void) {
    ...
    apiRepository.performRequest(path: Api.Endpoint.getUserActivity, method: .post, parameters: params) { (response, error) in
...
extension OtherPostsActivityViewController: UITableViewDelegate, UITableViewDataSource {
